...
\todo{Try to make the arches label more readable - Nicola}
\begin{figure}[!htbp]
    \centering
    \begin{tikzpicture}[
        squarednode/.style={rectangle, draw=black, very thick, minimum size=5mm},
...
To classify the EEG motor imagery data, we started from the literature review\todo{"started from the literature review" what does it mean -Nicola} and used a Long Short-Term Memory (LSTM) neural network based on the one presented in \cite{sharma_deep_2023}. 
We also tried creating a network based on their Transformer one, but it was not possible to train such network with the available hardware resources.\todo{I don't like how sentence 94 is put, try to rephrase -Nicola}
...
Since one of the most important applications of the model is wheelchair\todo{? wheelchair -Nicola} (or more generically personal devices) control, to test the classification model, we developed a virtual environment using the Unity game engine where the user could control a virtual avatar using their motor imagery EEG signals.

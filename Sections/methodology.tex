...
All these sections are part of a larger framework, that includes and merges all of them in a single pipeline that was developed as thesis project.
...
The path is a modular prefab~(Figure~\ref{fig:infinite_runner_path}), only the first two tiles are manually specified, the rest is generated randomly.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{Figures/Methodology/path_prefab}
    \caption{Infinite Runner Game path prefab.}
    \label{fig:infinite_runner_path}
\end{figure}
% \todo{check the image, not sure this fits}
